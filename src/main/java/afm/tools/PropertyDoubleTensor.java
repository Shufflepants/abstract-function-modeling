...
public class PropertyDoubleTensor extends DoubleTensor
{
    ...
    public DoubleTensor grad;

    public PropertyDoubleTensor(int[] sizes)
    {
        super(sizes);

    }

    public PropertyDoubleTensor(int[] sizes, ArrayList<Double>[] d)
    {
        super(sizes,d);
    }


    public DoubleTensor calcAttributes(DoubleTensor independent)
    {

        grad = new DoubleTensor(this.size);
        ...
        {
            index[i]++;
            domainArea = domainArea*(independent.getVectorAtIndex(index).get(i)-currentIndVector.get(i));
            index[i]--;
        }
        ...
        {
            partialDerivatives = new ArrayList<>();

            currentIndVector = independent.data[i];
            currentDepVector = this.data[i];
            ...
            for(int j=0;j<dim;j++)
            {
                ...
                if(index[j]+1 < size[j])
                {
                    ...
                    {
                        index[j]--;
                        x0 = independent.getVectorAtIndex(index).get(j);
                        y0 = getVectorAtIndex(index).get(0);

                        index[j] += 2;
                        x2 = independent.getVectorAtIndex(index).get(j);
                        y2 = getVectorAtIndex(index).get(0);
